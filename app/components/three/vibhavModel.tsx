...
export const Human = forwardRef<THREE.Group>((props, ref) => {
  const { scene, animations } = useGLTF(
    "https://vibhav-room.s3.eu-north-1.amazonaws.com/StrutWalking.glb"
  )
  const group = useRef<THREE.Group>(null)
  const hasReachedChair = useRef(false)
  const isWalking = useRef(false)
  ...
  useFrame((state, delta) => {
    ...
    if (
      isWalking.current &&
      actions.current["mixamo.com"] &&
      !actions.current["mixamo.com"].paused &&
      !hasReachedChair.current
    ) {
      const dir = new THREE.Vector3(0, 0, 1)
      dir.applyQuaternion(group.current.quaternion)
      dir.normalize()
      ...
      if (nextPos.distanceTo(chairPos) < 0.3) {
        // stop close enough
        group.current.position.copy(chairPos)
        isWalking.current = false
        hasReachedChair.current = true
...
// Preload the model
useGLTF.preload("https://vibhav-room.s3.eu-north-1.amazonaws.com/StrutWalking.glb")