...
const SunsetIcon = ({ className = "w-5 h-5" }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function GlassWeatherWidget({ isDarkMode = true }: GlassWeatherWidgetProps) {
  const hourlyData = [
  ...
  return (
    <div className={`w-80 h-40 p-4 flex flex-col justify-between select-none shrink-0 pointer-events-auto relative overflow-hidden transition-all duration-300 rounded-3xl ${
      ...
      <div className="relative z-10 flex justify-between items-start w-full">
        {/* Left Info */}
        <div>
          <div className={`flex items-center gap-1 text-[13px] font-bold leading-none ${textColorPrimary}`}>
            Weymouth
            {/* Location Navigation Arrow */}
            <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" className={isDarkMode ? 'text-white/85' : 'text-black/70'}>
        ...
        <div className="flex flex-col items-end text-right">
          ...
          <div className={`text-[12px] font-semibold leading-none ${textColorPrimary}`}>
            Mostly Sunny
          </div>
          <div className={`text-[10px] font-medium tracking-tight mt-1 leading-none ${textColorMuted}`}>
            H:78° L:54°
          </div>
        </div>
      </div>
      ...
      {/* Divider */}
      <div className={`relative z-10 h-[1px] my-2 ${isDarkMode ? 'bg-white/10' : 'bg-black/10'}`} />

      {/* Hourly Forecast */}
      <div className="relative z-10 flex justify-between items-center w-full px-1">
        {hourlyData.map((hour, idx) => (
          <div key={idx} className="flex flex-col items-center gap-1">
            <span className={`text-[9px] font-semibold uppercase leading-none ${textColorMuted}`}>
              {hour.time}
            </span>
            <div className={`my-0.5 ${isDarkMode ? 'text-white/90' : 'text-black/80'}`}>
              {hour.icon}
            </div>
            <span className={`text-[11px] font-bold leading-none ${isDarkMode ? 'text-white/90' : 'text-black/85'}`}>
              {hour.temp}
            </span>
          </div>
        ))}
