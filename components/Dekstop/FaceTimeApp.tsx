...
const CustomMenuIcon = ({ size = 16, className = "" }: { size?: number; className?: string }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const TrafficLights = ({ onClose, onMinimize, onMaximize, maximized = false }: TrafficLightsProps) => {
  return (
...
export default function FaceTimeApp({ onClose, onMinimize, onMaximize, maximized = false }: FaceTimeAppProps) {
  ...
  return (
    <div className="w-full h-full select-none text-[13px] rounded-xl overflow-hidden font-sans bg-[#121212] text-white relative">
      {/* Left Sidebar */}
      {isSidebarOpen && (
        <aside
        ...
        >
          ...
          <div className="p-4 pb-2 shrink-0 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button className={`p-1.5 rounded-lg transition flex items-center justify-center ${
              ...
              {contacts.map((contact) => (
                <div
                  ...
                  className={`rounded-[18px] flex flex-col h-[175px] transition-all duration-200 cursor-pointer relative group overflow-hidden ${
                    isDarkMode ? contact.bgDark : contact.bgLight
                  }`}
                >
                  <div className="px-3 pt-3 pb-1 shrink-0">
                    <span className={`font-semibold text-[12px] truncate block ${
                      isDarkMode ? "text-white" : "text-gray-900"
                    }`}>
                      {contact.name}
                    </span>
                  ...
                  <div className="flex-1 flex items-center justify-center py-1">
                    <div className="w-[78px] h-[78px] rounded-full overflow-hidden flex items-center justify-center shadow-md"
                      style={{ backgroundColor: contact.avatarBg }}
                    >
                      <img src={contact.avatar} alt={contact.name} className="w-[68px] h-[68px] object-contain" />
                    </div>
                  </div>
                    ...
                    {contact.sub === "Suggested" ? (
                      <>
                        ...
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition ${
                          isDarkMode ? "bg-white/10 group-hover:bg-white/20 text-white"
      ...
      {/* Main Video Screen */}
      <section className="absolute inset-0 w-full h-full z-0 overflow-hidden bg-black flex items-center justify-center">
        <video
          ref={videoRef}
          autoPlay
          playsInline
        ...
        {!isSidebarOpen && activeCall && (
          <>
            <div className="absolute top-4 left-4 z-20 flex items-center gap-2">
              ...
              <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[12px] font-semibold transition shadow-sm ${
            ...
            <div className="absolute bottom-5 left-4 z-20 w-[20%] min-w-[90px] max-w-[180px] rounded-2xl overflow-hidden border border-white/20 shadow-xl"
              style={{ backgroundColor: activeCall.avatarBg, aspectRatio: "1.6" }}>
              <img src={activeCall.avatar} alt={activeCall.name} className="w-full h-full object-contain" />
            </div>

            <div className="absolute bottom-5 right-5 z-20 flex items-center gap-2">
              <button className={`w-9 h-9 rounded-full flex items-center justify-center shadow-md transition active:scale-90 hover:brightness-105 ${
              ...
              <button className="w-9 h-9 rounded-full flex items-center justify-center shadow-md transition active:scale-90"
                style={{
                  backdropFilter: "blur(20px) saturate(180%)",
                  background: isDarkMode ? "rgba(255,255,255,0.15)" : "rgba(255,255,255,0.45)",
              ...
              <button onClick={endCall} className="w-9 h-9 rounded-full bg-[#ff3b30] hover:brightness-110 flex items-center justify-center text-white shadow-md transition active:scale-90">
