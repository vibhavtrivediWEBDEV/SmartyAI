...
export default function GlassClockWidget({ isDarkMode = true }: GlassClockWidgetProps) {
  ...
  function getRoundedRectPointAndNormal(d: number) {
    const halfS = S / 2;
    const p1 = halfS;
    const p2 = p1 + A;
    const p3 = p2 + S;
  ...
  for (let i = 0; i < 60; i++) {
    ...
    const textColor = isDarkMode ? 'rgba(255, 255, 255, 0.95)' : 'rgba(0, 0, 0, 0.85)';
    const inactiveColor = isDarkMode ? 'rgba(255, 255, 255, 0.2)' : 'rgba(0, 0, 0, 0.15)';
    
    // Ticks up to the current second are highlighted/active.
    const isActive = i <= currentSecond;
    const strokeColor = isActive ? textColor : inactiveColor;
  ...
  return (
    <div className={`w-40 h-40 flex flex-col items-center justify-center select-none shrink-0 pointer-events-auto relative overflow-hidden transition-all duration-300 rounded-3xl ${
      isDarkMode 
      ...
      <div className="relative z-10 flex flex-col items-center">
        <div 
          className={`text-[48px] font-black tracking-tight leading-none transition-colors duration-300 ${
            isDarkMode 
              ? 'text-white/95 drop-shadow-[0_2px_4px_rgba(0,0,0,0.15)]' 
              : 'text-black/90 drop-shadow-[0_1px_2px_rgba(255,255,255,0.5)]'
          }`}
          style={{
