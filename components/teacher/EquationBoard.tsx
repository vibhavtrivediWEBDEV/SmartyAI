...
import { sanitizeTeacherSpeech } from "@/modules/teaching/teacher-runtime"
...
export function EquationBoard({ items, activeItemIndex = null, subject, topic, visual, isLoadingVisual = false }: EquationBoardProps) {
  const [visibleCount, setVisibleCount] = useState(8)
  const [copied, setCopied] = useState(false)
  const itemRefs = useRef(new Map<number, HTMLElement>())
  const visible = useMemo(() => items.slice(-visibleCount), [items, visibleCount])
  const firstVisibleIndex = items.length - visible.length

  useEffect(() => {
    if (activeItemIndex === null) return
    itemRefs.current.get(activeItemIndex)?.scrollIntoView({ behavior: "smooth", block: "center" })
  ...
  return (
    <div className="flex h-full min-h-105 flex-col bg-[linear-gradient(145deg,#18191d,#101114)] p-2 sm:p-3">
      ...
      <div className="relative min-h-0 flex-1 overflow-y-auto rounded-b-2xl border-x border-b border-[#b88a52]/45 bg-[radial-gradient(circle_at_20%_10%,rgba(255,255,255,.035),transparent_28%),linear-gradient(135deg,#183d32,#0d2d25)] p-4 shadow-[inset_0_0_55px_rgba(0,0,0,.5),0_14px_35px_rgba(0,0,0,.3)] sm:p-6">
        <div aria-hidden className="pointer-events-none absolute inset-0 bg-[repeating-linear-gradient(0deg,transparent,transparent_31px,rgba(255,255,255,.25)_32px)] opacity-[.08]" />

        {!visible.length ? (
          <div className="relative flex h-full min-h-80 flex-col items-center justify-center text-center">
            <div className="mb-4 flex size-14 items-center justify-center rounded-2xl border border-white/10 bg-white/5"><Eraser className="size-7 text-[#f4e6c1]/70" /></div>
          ...
          <div className={`relative grid gap-4 text-left ${visual || isLoadingVisual ? "xl:grid-cols-[minmax(0,1fr)_280px]" : ""}`}>
            <div className="space-y-4">
              {visible.map((item, index) => {
                ...
                return (
                <article ref={(element) => { if (element) itemRefs.current.set(itemIndex, element); else itemRefs.current.delete(itemIndex) }} key={`${item.latex}-${itemIndex}`} aria-current={isActive ? "step" : undefined} className={`rounded-xl border p-4 shadow-sm transition motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 ${isActive ? "border-[#ffd60a]/70 bg-[#ffd60a]/10 ring-2 ring-[#ffd60a]/25" : "border-white/8 bg-black/10"}`} style={{ animationDelay: `${Math.min(index * 70, 350)}ms` }}>
                  ...
                  <button onClick={() => window.speechSynthesis?.speak(new SpeechSynthesisUtterance(sanitizeTeacherSpeech(item.explanation)))} className="mt-3 flex items-center gap-1.5 rounded-lg px-2 py-1 text-[10px] text-white/35 transition hover:bg-white/[.07] hover:text-white/60"><Volume2 className="size-3" /> Read explanation aloud</button>
                </article>
              )})}
              {items.length > visibleCount && <button onClick={() => setVisibleCount((count) => count + 8)} className="w-full rounded-xl border border-white/10 bg-white/4 py-2 text-xs text-white/45 hover:bg-white/7">Show earlier board notes</button>}
            </div>
